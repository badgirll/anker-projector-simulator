...
function drawTopView(screenWidth, screenHeight, throwDistanceM, screenDiagonal, isError = false, lumens = 2000) {
    ...
    const topBottomPadding = isMobile ? 140 : 80; // Extra space on mobile for top label
...
function drawFrontView(screenWidth, screenHeight, screenDiagonal, isError = false, lumens = 2000) {
    ...
    const viewBoxWidth = isMobile ? 1100 : 900; // Much wider on mobile to prevent right side clipping
    const viewBoxStartX = isMobile ? -300 : 0; // Shift far left on mobile to show wall height label
    ...
    const targetWallWidth = viewBoxWidth * (isMobile ? 0.7 : 0.85); // Smaller percentage on mobile for better margins
