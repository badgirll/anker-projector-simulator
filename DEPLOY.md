...
### 4. Vercel Analytics を有効化（オプション）

**重要**: vercel.json で設定するとスキーマエラーになります。ダッシュボードから設定してください。

1. Vercel ダッシュボードでプロジェクトを選択
2. 「Analytics」タブをクリック
3. 「Enable Analytics」をクリック

**注意**: `vercel.json` に `analytics` プロパティを追加すると以下のエラーが発生します：
```
...
1. ogp.png がプロジェクトルートに配置されているか確認
2. index.html のメタタグの URL が正しいか確認
3. Facebook シェアデバッガーでキャッシュをクリア: https://developers.facebook.com/tools/debug/
4. Twitter Card Validator で確認: https://cards-dev.twitter.com/validator

### Vercel Analytics のエラー

- `vercel.json` で `analytics` プロパティを設定するとデプロイエラーになります
- 解決方法: `vercel.json` を削除し、Vercel ダッシュボードから手動で有効化してください
